...
import { InstrumentType, InstrumentTypes, CommissionMode, CommissionModeReverse } from 'kungfu-shared/config/tradingConfig';
...
export const addCommission = async () => {
    ...
    try {
        ...
        if (!addCommissionItem.product_id || !addCommissionItem.exchange_id) {
            throw new Error("Lack the required Data, ReAdd the Commission Setting Item")
        }

        const existedIndex: number = oldCommissionList.findIndex((item: CommissionItem) => {
...
export const removeCommission = async () => {
    try {
        ...
        const productId = resultResolved[0].split(" ").length === 1 ? "" : resultResolved[0].split(" ")[1];
        const exchangeId = resultResolved[1].split(" ").length === 1 ? "" : resultResolved[1].split(" ")[1];
