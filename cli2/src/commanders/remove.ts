...
export const removeAccountStrategy = async () => {
  ...
  let answers = await inquirer.prompt([
    {
      ...
      source: async (answersSoFar: any, input: string) => {
        input = input || '';
        return accountStrategyList.filter((s: string) => s.includes(input));
      },
    },
  ]);
  ...
  try {
    switch (type) {
      case 'md':
        const targetMd =
          mds.filter((m: Md) => m.source_name === targetId)[0] || {};
        await deleteMd(targetMd);
        console.success(
          `Delete ${colors.yellow(type)} ${colors.bold(targetId)}`,
        );
        break;
      case 'td':
        const targetTd =
          tds.filter((a: Td) => a.account_id === targetId)[0] || {};
