import {
  SystemConfigItem,
  SystemConfigChildNormalItem,
  mergeSystemConfigTogether,
  getValueByConfigItem,
  setValueByConfigItem,
} from '__gConfig/systemConfig';
  ...
  return inquirer.prompt([
    {
      ...
      source: async (answersSoFar: any, input = '') => {
        return Object.values(systemConfig).map(
          (systemConfigItem: SystemConfigItem) => systemConfigItem.key,
        );
      },
    },
  ]);
...
const buildChoices = async (configItem: SystemConfigChildNormalItem) => {
  ...
  switch (targetType) {
    case 'select':
      return (configItem.options || configItem.data || []).map(
        (item: SystemConfigChildSelectItemData) => item.value,
      );
    case 'sources':
      const sourceList: SystemConfigChildSelectItemData[] =
        await getSourceList();
      return sourceList;
    default:
      return [];
...
const parseConfigItemQuestions = async (
  configItem: SystemConfigChildNormalItem,
  existedConfigData: StringToAnyObject,
...
export const setSystemConfig = async () => {
  const configItemKey = (await promptQuestionForSelectTargetConfigItem())
    .systemConfigTarget;
  const systemConfigOfTarget = systemConfig[configItemKey];
  const configType = systemConfigOfTarget.configType || '';
  const configChildItems = systemConfigOfTarget.config.filter(
    (config: SystemConfigChildNormalItem) => !!config.cli,
  );
  const configValueData = getValueByConfigItem(configType, configItemKey);
  const questions = await Promise.all(
    configChildItems.map((cItem: SystemConfigChildNormalItem) =>
      parseConfigItemQuestions(cItem, configValueData),
    ),
  );
  return inquirer
    .prompt(questions)
    .then((answers: StringToAnyObject) =>
      setValueByConfigItem(configType, configItemKey, answers),
    );
};
