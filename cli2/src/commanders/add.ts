...
import {
  ...
  getExistedMdSources,
  updateMdConfig,
} from '__io/kungfu/account';
import {
  getStrategyList,
  addStrategy,
  updateStrategyPath,
} from '__io/kungfu/strategy';
import {
  parseSources,
  renderSelect,
  getQuestionInputType,
  getKungfuTypeFromString,
...
const getAvailableSources = (
  accountSource: Sources,
  existedSource: string[],
): string[] => {
...
const selectSourcePrompt = async (
  accountSource: Sources,
  existedSource: string[],
) => {
  ...
  return inquirer.prompt([
    {
      ...
      source: async (answersSoFar: any, input = '') => {
        return availSources.filter((s: string) => s.includes(input));
      },
    },
  ]);
};

export const accountConfigPrompt = (
  ...
  const questions = accountOptions
    .filter(
      (a: AccountSettingItem) =>
        type === 'md' || !(updateModule && a.key === idKey),
    )
    .map((a: AccountSettingItem) =>
      paresAccountQuestion(
        {
          source,
          idKey,
          configItem: a,
...
export const addUpdateTdByPrompt = async (
  source: string,
  key: string,
  config: any,
  updateModule = false,
  ...
  try {
    if (updateModule) {
      ...
      console.success(
        `Update ${colors.cyan('td')} ${colors.bold(accountId)} ${JSON.stringify(
          config,
          null,
          '',
        )}`,
      ...
      console.success(
        `Add ${colors.cyan('td')} ${colors.bold(accountId)} ${JSON.stringify(
          config,
          null,
          '',
        )}`,
...
export const addUpdateMdByPrompt = async (
  source: string,
  config: any,
  updateModule = false,
) => {
  try {
    if (updateModule) {
      ...
      console.success(
        `Update ${colors.yellow('md')} ${colors.bold(source)} ${JSON.stringify(
          config,
          null,
          '',
        )}`,
      ...
      console.success(
        `Add ${colors.yellow('md')} ${colors.bold(source)} ${JSON.stringify(
          config,
          null,
          '',
        )}`,
...
const addMdPrompt = (accountSource: Sources) => {
  return getExistedMdSources()
    .then((sourcesList: string[]) =>
      selectSourcePrompt(accountSource, sourcesList),
    )
    .then(({ source }: { source: string }) => source.split(' ')[0])
    .then((source: string) => {
      const accountSetting: AccountSetting = accountSource[source];
...
// ======================= add strategy start ============================
export const addUpdateStrategyPrompt = async (
  strategyData?: any,
  updateModule?: boolean,
): Promise<any> => {
  const { strategy_id, strategy_path } = await inquirer.prompt(
    buildStrategyQuestion(strategyData, updateModule).filter((q) => !!q),
  );
  ...
  try {
    if (updateModule) {
      ...
      console.success(
        `Update ${colors.blue('strategy')} ${colors.bold(
          strategyId,
        )} ${strategy_path}`,
      );
    } else {
      ...
      console.success(
        `Add ${colors.blue('strategy')} ${colors.bold(
          strategy_id,
        )} ${strategy_path}`,
      );
    }
...
function buildStrategyQuestion(
  strategyData: any,
  updateModule: boolean | undefined,
) {
  if (updateModule) {
    return [
      {
        ...
        validate: (value: string) => {
          let hasError = null;
          !updateModule &&
            requiredValidator(null, value, (err: Error) => (hasError = err));
  ...
  return [
    {
      ...
      validate: async (value: string) => {
        ...
        existedIds.includes(value) &&
          (hasError = new Error('Strategy_id has existed!'));
  ...
  let questions = {
    ...
    choices:
      targetType === 'list'
        ...
        : [],
    message: `${updateModule ? 'Update' : 'Enter'} ${key} ${renderSelect(
      configItem,
    )}`,
    validate: async (value: string | number) => {
      ...
      if (idKey === key && type !== 'md') {
        const existedAccountError = await existedAccountIdValidator(
          source,
          value,
        );
...
async function existedAccountIdValidator(
  source: string,
  value: any,
): Promise<any> {
  ...
  if (existedIds.includes(currentAccountId))
    return new Error(`AccountId ${currentAccountId} has existed!`);
}
