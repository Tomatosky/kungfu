import {
  ...
  addUpdateTdByPrompt,
  addUpdateMdByPrompt,
  addUpdateStrategyPrompt,
  filterAccountConfig,
} from '@/commanders/add';
import {
  getAccountsStrategys,
  accountStrategyListStringify,
} from '@/assets/scripts/actions/accountStrategyActions';
...
export const updateAccountStrategy = async () => {
  ...
  const accountStrategyList = accountStrategyListStringify(
    mds,
    tds,
    strategies,
  );
  const answers = await inquirer.prompt([
    {
      ...
      source: async (answersSoFar: any, input: string) => {
        input = input || '';
        return accountStrategyList.filter((s: string): boolean =>
          s.includes(input),
        );
      },
    },
...
async function updateTd(targetId: string, tds: Td[]) {
  ...
  const { key, config } = await accountConfigPrompt(
    accountSetting,
    true,
    tdConfig,
  );
...
async function updateStrategy(targetId: string, strategies: Strategy[]) {
  const targetStrategy = strategies.filter(
    (s: Strategy) => s.strategy_id === targetId,
  );
