...
import {
  listProcessStatusWithDetail,
  startArchiveMakeTask,
} from '__gUtils/processUtils';
...
export const switchProcess = (proc: any, messageBoard: any, loading: any) => {
  ...
  if (!status) {
    if (proc.processId !== 'master') {
      if (!watcher.isLive()) {
        messageBoard.log(
          `Watcher is not alive! Start master and waitting for a while.`,
          2,
        );
        return;
      }
    ...
    case 'strategy':
      ...
      switchStrategy(proc.processId, !status)
        .then(() => {
          loading.stop();
          return messageBoard.log(
            `${startOrStop} Strategy process success!`,
            2,
          );
        })
        .catch((err: Error) => logger.error(err));
    ...
    default:
      ...
      switchCustomProcess(!status, processId)
        .then(() => {
          loading.stop();
          return messageBoard.log(
            `${startOrStop} ${processId.toUpperCase()} process success!`,
            2,
          );
        })
        .catch((err: Error) => logger.error(err));
...
export const processListObservable = () =>
  combineLatest(
    ...
    (
        ...
        a[mdProcessId] = {
          ...
          statusName: dealStatus(
            buildTdMdStatus(
              mdProcessId,
              mdTdState,
              buildStatusDefault(processStatus[mdProcessId]).status,
            ),
          ),
        ...
        a[tdProcessId] = {
          ...
          statusName: dealStatus(
            buildTdMdStatus(
              tdProcessId,
              mdTdState,
              buildStatusDefault(processStatus[tdProcessId]).status,
            ),
          ),
      ...
      const strategyData = [{}, ...strategyList].reduce(
        (a: any, b: any): any => {
          ...
          a[strategyProcessId] = {
            ...
            type: 'strategy',
            statusName: dealStatus(
              buildStatusDefault(processStatus[strategyProcessId]).status,
            ),
      ...
      return [
        {
          ...
          type: 'main',
          statusName: dealStatus(
            buildStatusDefault(processStatus['archive']).status,
          ),
        ...
        },
        {
          ...
          type: 'main',
          statusName: dealStatus(
            buildStatusDefault(processStatus['master']).status,
          ),
        ...
        },
        {
          ...
          type: 'main',
          statusName: dealStatus(
            buildStatusDefault(processStatus['ledger']).status,
          ),
...

        {
          ...
          type: 'calc',
          statusName: dealStatus(
            buildStatusDefault(processStatus['bar']).status,
          ),
