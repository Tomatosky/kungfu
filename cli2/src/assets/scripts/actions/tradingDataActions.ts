...
export const tradingDataObservale = (type: string, processId: string) => {
  ...
  return buildKungfuDataByAppPipe().pipe(
    map(() => {
      ...
      const positions = ensureLedgerData(ledgerData.Position).map(
        (item: PosOriginData) => dealPos(item),
      );
      const assets = ensureLedgerData(ledgerData.Asset).map(
        (item: AssetOriginData) => dealAsset(item),
      );
      ...
      if (type === 'account') {
        ...
        const ordersResolved = dealOrdersFromWatcher(
          orders,
          orderStatResolved,
          dealOrderStat,
        );
        const tradesResolved = dealTradesFromWathcer(
          trades,
          orderStatResolved,
          dealOrderStat,
        );
        const positionsResolved =
          transformTradingItemListToData(positions, 'account')[processId] || [];
        const assetsResolved =
          transformAssetItemListToData(assets, 'account')[processId] || [];
        ...
        const positionsResolved =
          transformTradingItemListToData(positions, 'strategy')[processId] ||
          [];
        const assetsResolved =
          transformAssetItemListToData(assets, 'strategy')[processId] || [];
...
function dealOrdersFromWatcher(
  orders: OrderData[],
  orderStatOrigin: { [prop: string]: OrderStatOriginData },
  dealOrderStat: any,
) {
...
function dealTradesFromWathcer(
  trades: TradeData[],
  orderStatOrigin: { [prop: string]: OrderStatOriginData },
  dealOrderStat: any,
) {
