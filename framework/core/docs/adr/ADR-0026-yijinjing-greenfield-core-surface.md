...
- `longfist.h` still carried `TradingDataTypes` and `MarketDataTypes` registry
  names that invited new code to copy a trading closed-set model.
...
- The old generated longfist schema files are not fully deleted by this ADR.
  They remain until a later legacy/trading split removes or moves the compiled
...
- **Delete every trading schema in one change.** Rejected for this slice because
  `types.h`, config/profile caches, and Node watcher compatibility still need a
  staged split. The first invariant is to stop exposing and extending them as
  core yijinjing APIs.
