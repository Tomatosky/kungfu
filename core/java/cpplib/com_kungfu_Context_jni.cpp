...
JNIEXPORT void JNICALL Java_com_kungfu_Context_add_1account
  (JNIEnv *env, jobject self, jstring source, jstring account, jdouble cashLimit){
  ...
  extern "C" JNIEXPORT jlong JNICALL Java_com_kungfu_Context_insert_1order(
    ...
    uint64_t oid = _self->insert_order(str_instrument_id, str_exchange_id, str_account, d_limit_price, i64_volume, price_type, side,
                        offset, hedge);
    std::cout << "oid=" << uint64_t(oid) << std::endl;
    return (jlong)oid;
  }

  extern "C" JNIEXPORT jlong JNICALL Java_com_kungfu_Context_cancel_1order
  (JNIEnv *env, jobject self, jlong order_id){
      Context *_self = getObject(env, self);
