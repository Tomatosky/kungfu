...
KF_DEFINE_PACK_TYPE(                              //
    ...
    (double, avail_margin),    //可用保证金，这一项要显示到前端
    (double, rz_debt), // 融资负债
...
KF_DEFINE_PACK_TYPE(                               //
    ...
    (double, rq_market_value), // 融券卖出证券市值(欠券): 融券负债 -(手续费+税费) --- 随行情实时变化，仅使用一次，买券还券时记录closed_rq_debt = trade.price * trade.volume;
    (double, rq_income), // 融券负债 (累计 融券卖出清算金额=产生成交时的融券卖出金额-(手续费+税费))，与total_asset相关
    // (double, rz_margin), // 融资占用保证金
    (double, rq_margin), // 融券占用保证金

    (double, credit), //信用额度 CreditQuota （总可负债额度）
    // (double, avail_credit), //可用信用额度 （当前可负债额度） : 可用保证金 (dKYBZJ)  不是一个概念!   信用负债可以达到
    // 保证金额度/margin_ratio. (double, frozen_credit), //冻结信用额度 vs  冻结保证金(两融) use frozen_margin
