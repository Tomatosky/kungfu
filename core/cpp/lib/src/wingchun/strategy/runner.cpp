...
void Runner::post_start() {
  ...
  events_ | is_own<Entrust>(context_->get_broker_client()) | $$(invoke(&Strategy::on_entrust, event->data<Entrust>()));
  ...
  events_ | is_own<Deregister>(context_->get_broker_client()) | $$(invoke(&Strategy::on_deregister, event->data<Deregister>()));
  events_ | is_own<BrokerStateUpdate>(context_->get_broker_client()) | $$(invoke(&Strategy::on_broker_state_change, event->data<BrokerStateUpdate>(), context_->get_broker_client().get_location(event->source())));
