<template>
  <tr-dashboard :title="title || '策略'">
    <div slot="dashboard-header">
      ...
      <tr-dashboard-header-item>
        <i
          class="el-icon-refresh mouse-over"
          title="刷新"
          @click="handleRefresh"
        ></i>
      </tr-dashboard-header-item>
      <tr-dashboard-header-item v-if="!value && !isAdmin">
        <i
          class="el-icon-monitor mouse-over"
          title="打开监控"
          @click="handleMonitStrategies"
        ></i>
      </tr-dashboard-header-item>
      <tr-dashboard-header-item v-else-if="!isAdmin">
        <i
          class="el-icon-s-platform mouse-over"
          title="关闭监控"
          @click="handleMonitStrategies"
        ></i>
      </tr-dashboard-header-item>
      <tr-dashboard-header-item>
        <el-button size="mini" @click="handleAddStrategy" title="添加">
          添加
        </el-button>
      </tr-dashboard-header-item>
    </div>
    <div class="table-body">
      ...
      >
        ...
        <el-table-column label="进程" sortable v-if="!isAdmin">
        ...
        >
          <template slot-scope="props">
            <tr-blink-num
              :theKey="`realized_pnl_${props.row.strategy_id}_${calcCash(
                props.row,
                'realizedPnl',
              )}`"
        ...
        >
          <template slot-scope="props">
            <tr-blink-num
              :theKey="`unrealized_pnl_${props.row.strategy_id}_${calcCash(
                props.row,
                'unRealizedPnl',
              )}`"
        ...
        >
          <template slot-scope="props">
            <tr-blink-num
              :theKey="`${props.row.strategy_id}_${calcCash(
                props.row,
                'marketValue',
              )}`"
        ...
        >
          <template slot-scope="props">
            <span
              class="tr-oper"
              @click.stop="handleOpenLogFile(props.row.strategy_id)"
            >
              <i class="el-icon-document mouse-over" title="打开日志文件"></i>
            </span>
            <span class="tr-oper" @click.stop="handleSetStrategy(props.row)">
              <i class="mouse-over el-icon-setting"></i>
            </span>
            <span class="tr-oper" @click.stop="handleEditStrategy(props.row)">
              <i class="mouse-over el-icon-edit-outline"></i>
            </span>
            <span
              class="tr-oper-delete"
              @click.stop="handleDeleteStrategy(props.row)"
            >
              <i class="mouse-over el-icon-delete"></i>
            </span>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      ...
      >
        ...
        >
          <span
            class="path-selection-in-dialog text-overflow"
            :title="setStrategyForm.strategyPath"
          >
            {{ setStrategyForm.strategyPath }}
          </span>
          <el-button
            size="mini"
            icon="el-icon-more"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" @click="handleClearAddStrategyDialog">
          取 消
        </el-button>
        <el-button
          type="primary"
          size="mini"
          @click="handleConfirmAddEditorStrategy"
        >
          确 定
        </el-button>
      </div>
...
import {
  ...
  noKeywordValidatorBuilder,
} from '__assets/validator';
...
export default {
  ...
  methods: {
    ...
    handleDeleteStrategy(row) {
      ...
      const strategyProcessStatus = ifProcessRunning(
        strategy_id,
        this.processStatus,
      );
      ...
      this.$confirm(
        `删除策略 ${strategy_id} 会删除所有相关信息，确认删除吗？`,
        '提示',
        ...
        .then(() => {
          ...
          if (this.currentStrategy.strategy_id === strategy_id) {
            const currentStrategy = this.strategyList.length
              ? this.strategyList[0]
              : {};
    ...
    handleEditStrategy(row) {
      openVueWin('code', `/kungfuCodeEditor/${row.strategy_id}`, remote);
    },
      ...
      this.$refs['setStrategyForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.setStrategyDialogType === 'add') {
            firstStepPromise = this.$confirm(
              `添加后策略ID不能更改，确认添加 ${strategy} 吗？`,
              '提示',
              {
          ...
          firstStepPromise.then(() => {
            ...
            const strategyMethod =
              this.setStrategyDialogType === 'add'
                ? STRATEGY_API.addStrategy
                : STRATEGY_API.updateStrategyPath;
            strategyMethod(strategy, strategyPath)
              .then(() => this.getStrategyList()) //get new list
              .then(() => {
                this.$message.success(
                  (this.setStrategyDialogType === 'add' ? '添加' : '修改') +
                    '成功！',
                );
    ...
    validateDuplicateStrategyId(rule, value, callback) {
      const ifDuplicate =
        this.strategyList.filter((s) => s.strategy_id === value).length !== 0;
    ...
    calcCash(row, key) {
      return (
        toDecimal((this.strategyiesAsset[row.strategy_id] || {})[key]) + ''
      );
    },
  },
};
