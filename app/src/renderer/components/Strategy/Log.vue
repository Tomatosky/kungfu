    ...
    <div slot="dashboard-header">
      ...
      <tr-dashboard-header-item>
        <i
          ...
          title="打开日志文件"
          @click="handleOpenLogFile(logPath, true)"
        ></i>
      </tr-dashboard-header-item>
      <tr-dashboard-header-item>
        <el-button size="mini" @click="handleClearLog" title="清空">
          清空
        </el-button>
      </tr-dashboard-header-item>
    </div>
  ...
  methods: {
    ...
    bindGetLogWorkerListener() {
      ...
      Workers.getLogWorker.onmessage = (event) => {
        const { list, logPath, searchKeyword } = event.data || [];
    ...
    startWatchingTail(processId, logPath, searchKeyword) {
      ...
      this.tailObserver.on('line', (line) =>
        ((curProcId, curKw) => {
          if (curKw) return;
