    ...
    <div class="account-status-content">
      <div class="account-item">
        ...
        <div
          ...
          v-for="accountItem in mdList"
          :key="accountItem.source_name"
        >
          ...
          <span class="account-process-item text-overflow" style="width: 81px">
            <tr-status
              v-if="
                ifProcessRunning(
                  `md_${accountItem.source_name}`,
                  processStatus,
                ) && processStatus[`md_${accountItem.source_name}`] === 'online'
              "
              :value="(mdTdState[`md_${accountItem.source_name}`] || {}).state"
            ></tr-status>
            <tr-status
              v-else-if="
                ifProcessRunning(
                ...
                processStatus[`md_${accountItem.source_name}`] === 'stopping'
              "
              :value="processStatus[`md_${accountItem.source_name}`]"
            ></tr-status>
          ...
          <span class="account-process-item status-switch" @click.stop>
            <el-switch
              :value="
                ifProcessRunning(`md_${accountItem.source_name}`, processStatus)
              "
              @change="handleMdSwitch($event, accountItem)"
            ></el-switch>
          </span>
          <span
            class="account-process-item status-switch"
            @click.stop="handleOpenLogFile(`md_${accountItem.source_name}`)"
          >
      ...
      <div class="account-item">
        ...
        >
          ...
          <span
            class="account-process-item text-overflow"
            :title="accountItem.account_id.toAccountId()"
          >
          ...
          <span class="account-process-item text-overflow" style="width: 81px">
            <tr-status
              v-if="
                ifProcessRunning(
                  `td_${accountItem.account_id}`,
                  processStatus,
                ) && processStatus[`td_${accountItem.account_id}`] === 'online'
              "
              :value="(mdTdState[`td_${accountItem.account_id}`] || {}).state"
            ></tr-status>
            <tr-status
              v-else-if="
                ifProcessRunning(
                ...
                processStatus[`td_${accountItem.account_id}`] === 'stopping'
              "
              :value="processStatus[`td_${accountItem.account_id}`]"
            ></tr-status>
            <tr-status v-else></tr-status>
          </span>
          ...
          <span class="account-process-item status-switch">
            <el-switch
              :value="
                ifProcessRunning(`td_${accountItem.account_id}`, processStatus)
              "
              @change="handleTdSwitch($event, accountItem)"
            ></el-switch>
          </span>
          <span
            class="account-process-item status-switch"
            @click.stop="handleOpenLogFile(`td_${accountItem.account_id}`)"
          >
            <i class="el-icon-document mouse-over" title="打开日志文件"></i>
          </span>
        </div>
...
export default {
  ...
  computed: {
    ...
    currentStatus() {
      ...
      this.tdList.map((a) => {
        const tdProcessStatus = this.ifProcessRunning(
          'td_' + a.account_id,
          this.processStatus,
        );
        ...
        if (true) {
          const mdProcessStatus = this.ifProcessRunning(
            'md_' + a.source_name,
            this.processStatus,
          );
  ...
  methods: {
    ...
    handleTdSwitch(value, account) {
      switchTd(account, value).then(({ type, message }) =>
    ...
    handleMdSwitch(value, account) {
      switchMd(account, value).then(({ type, message }) =>
        this.$message[type](message),
      );
    },
  },
