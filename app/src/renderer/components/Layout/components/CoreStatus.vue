    ...
    <div class="core-status-content">
      <div class="core-item">
        <div class="core-status">
          <span
            class="core-process-item core-process-title text-overflow"
            title="主进程"
          >
            主控进程
            <el-tag type="warning">Master</el-tag>
          </span>
          <span class="core-process-item text-overflow" style="width: 71px">
            <tr-status
              v-if="ifProcessRunning('master', processStatus)"
              :value="buildState('master')"
            ></tr-status>
            <tr-status v-else></tr-status>
          </span>
          <span
            class="core-process-item switch"
            v-if="NODE_ENV === 'development'"
          >
            <el-switch
              :value="ifProcessRunning('master', processStatus)"
          ...
          <span class="core-process-item get-log">
            <i
              class="el-icon-document mouse-over"
              title="打开日志文件"
              @click="handleOpenLogFile('master')"
            ></i>
          </span>
        </div>
      </div>
      <div class="core-item">
        <div class="core-status">
          ...
          >
            数据进程
            <el-tag>Ledger</el-tag>
          </span>
          <span class="core-process-item text-overflow" style="width: 71px">
            <tr-status
              v-if="ifProcessRunning('ledger', processStatus)"
              :value="buildState('ledger')"
            ></tr-status>
            <tr-status v-else></tr-status>
          </span>
          <span
          ...
          >
            <el-switch
              :value="ifProcessRunning('ledger', processStatus)"
              @change="handleLedgerSwitch"
            ></el-switch>
          </span>
          <span class="core-process-item text-overflow monit">
          ...
          <span class="core-process-item get-log">
            <i
              class="el-icon-document mouse-over"
              title="打开日志文件"
              @click="handleOpenLogFile('ledger')"
            ></i>
          </span>
        </div>
      </div>
      <div class="core-item">
        <div class="core-status">
          ...
          >
            通信进程
            <el-tag>Daemon</el-tag>
          </span>
          <span class="core-process-item text-overflow" style="width: 71px">
            <tr-status
              v-if="ifProcessRunning('kungfuDaemon', processStatus)"
              :value="buildState('kungfuDaemon')"
            ></tr-status>
            <tr-status v-else></tr-status>
          </span>
          <span
          ...
          >
            <el-switch
              :value="ifProcessRunning('kungfuDaemon', processStatus)"
              @change="handleDaemonSwitch"
            ></el-switch>
          </span>
          <span class="core-process-item text-overflow monit">
          ...
          <span class="core-process-item text-overflow monit">
            MEM:{{
              getMemCpu('kungfuDaemon', processStatusWithDetail, 'memory')
            }}
          </span>
          <span class="core-process-item get-log">
            <i
              class="el-icon-document mouse-over"
              title="打开日志文件"
              @click="handleOpenLogFile('kungfuDaemon')"
            ></i>
          </span>
        </div>
      </div>
      ...
      if (!ifProcessRunning('kungfuDaemon', this.processStatus)) {
        if (!this.daemonErrController && !!this.processStatus['kungfuDaemon']) {
          this.$message.warning(
            '通信进程断开，无法监控交易数据，但交易仍将正常进行，如有需要请重启应用！',
            0,
          );
