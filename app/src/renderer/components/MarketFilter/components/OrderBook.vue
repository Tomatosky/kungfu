    ...
    <div class="kf-order-book__warp">
      <div class="ask__warp order-book-part__warp">
        ...
        >
          <span
            class="volume buy"
            @click.stop="handleMakeOrder(askPrices[9 - index], 0)"
          ></span>
          <span class="price green">{{ dealNum(askPrices[9 - index]) }}</span>
          <span
            class="volume sell"
            @click.stop="handleMakeOrder(askPrices[9 - index], 1)"
          >
            {{ dealNum(askVolumes[9 - index]) }}
          </span>
        </div>
      </div>
      ...
      <div class="bid__warp order-book-part__warp">
        ...
        >
          <span
            class="volume buy"
            @click.stop="handleMakeOrder(bidPrices[index], 0)"
          >
            {{ dealNum(bidVolumes[index]) }}
          </span>
          <span class="price red">{{ dealNum(bidPrices[index]) }}</span>
          <span
            class="volume sell"
            @click.stop="handleMakeOrder(bidPrices[index], 1)"
          ></span>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    askPrices() {
      ...
      return (this.quoteData.askPrices || []).map((price, index) => {
    ...
    askVolumes() {
      if (!this.quoteData) return [];
      return (this.quoteData.askVolumes || []).map((volume) => {
    ...
    bidPrices() {
      ...
      return (this.quoteData.bidPrices || []).map((price, index) => {
        const volume = volumes[index] || 0;
        if (price < MIN_PRICE || volume > MAX_VOUME || volume <= 0) {
          return 0;
        }
    ...
    bidVolumes() {
      if (!this.quoteData) return [];
      return (this.quoteData.bidVolumes || []).map((volume) => {
        if (volume > MAX_VOUME || volume <= 0) {
          return 0;
        }
        return volume;
