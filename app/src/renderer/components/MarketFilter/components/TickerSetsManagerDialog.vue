<template>
  ...
  >
    <div class="kf-tickersets-manager-dialog__warp">
      <TickerSets :editingTickerSet.sync="editingTickerSet"></TickerSets>
      <div class="kf-tickerset-detail_warp">
        <tr-dashboard
          :title="`标的池：${(editingTickerSet || {}).name || '需添加标的池'}`"
        >
          <div slot="dashboard-header">
            <tr-dashboard-header-item>
              <el-button size="mini" @click="handleAddTicker" title="添加">
                添加标的
              </el-button>
            </tr-dashboard-header-item>
          </div>
          <div class="table-body ticker-set-table">
            ...
            >
              ...
              <el-table-column
                ...
                show-overflow-tooltip
              ></el-table-column>
              <el-table-column prop="source" label="柜台" show-overflow-tooltip>
              ...
              >
                <template slot-scope="props">
                  <span class="tr-oper-delete">
                    <i
                      class="el-icon-delete mouse-over"
                      title="删除"
                      @click.stop="
                        handleDeleteTicker(props.row, editingTickerSet)
                      "
                    ></i>
                  </span>
                </template>
...
export default {
  ...
  computed: {
    ...
    tickersInEditingTickerSet() {
      ...
      const target = findTargetFromArray(
        this.tickerSets,
        'name',
        editingTickerSetName,
      );
      if (target) {
        return (target.tickers || []).slice(0).sort((ticker1, ticker2) => {
