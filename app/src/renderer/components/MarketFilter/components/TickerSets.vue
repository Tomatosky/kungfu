  ...
  <div class="kf-ticker-set__warp">
    <tr-dashboard title="标的池列表">
      <div slot="dashboard-header">
        ...
        <tr-dashboard-header-item>
          <el-button size="mini" @click="handleAddTickerSet" title="添加">
            添加标的池
          </el-button>
        </tr-dashboard-header-item>
      </div>
      <div class="table-body ticker-set-table">
        ...
        >
            ...
            <template slot-scope="props">
              ...
              <span
                v-if="
                  ...
                  props.row.name === currentTickerSetName
                "
                title="双击重命名"
                @dblclick.stop="handleChangeName(props.row)"
              >
          ...
          >
            <template slot-scope="props">
              <span class="tr-oper-edit" v-if="props.row.name !== 'editing'">
                <i
                  class="el-icon-edit mouse-over"
                  title="重命名"
                  @click.stop="handleChangeName(props.row)"
                ></i>
              </span>
              <span class="tr-oper-delete" v-if="props.row.name !== 'editing'">
                <i
                  class="el-icon-delete mouse-over"
                  title="删除"
                  @click.stop="handleRemoveTickerSet(props.row)"
                ></i>
              </span>
            </template>
          </el-table-column>
...
export default {
  ...
  watch: {
    tickerSets(newTickerSets) {
      ...
      if (!this.tableList.length) {
        ...
        !findTargetFromArray(this.tableList, 'name', editingTickerSetName)
      ) {
  ...
  methods: {
    ...
    handleBlurEditingNewTickerSet(index) {
      ...
      if (
        targetTicker.editingType !== 'new' &&
        targetTicker.editingType !== 'rename'
      )
        return;
      ...
      if (targetTicker.editingType === 'new') {
        ...
        return this.handleConfirmAddSetTickerSet(tickerSet).then(() =>
          this.updateEditingTickerSet(tickerSet),
        );
      }
