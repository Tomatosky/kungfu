<template>
  <div class="day-chart">
    <div class="day-current pnl-statis">
      ...
      <div v-if="accumulatedPnl !== ''">
        累计收益：
        <span
          :class="{
            'text-overflow': true,
            'color-green': accumulatedPnl < 0,
            'color-red': accumulatedPnl > 0,
          }"
          :title="accumulatedPnl"
        >
          {{ accumulatedPnl }}
        </span>
      </div>
    ...
    <tr-no-data
      v-if="accumulatedPnl === '' || dailyPnl.length == 0 || !rendererPnl"
    />
...
export default {
  ...
  watch: {
    ...
    dailyPnl(dailyPnlList, oldPnlMinList) {
      const { timeList, pnlDataList } = this.dealDailyPnlList(
        dailyPnlList.slice(0),
      );
