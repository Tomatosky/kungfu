<template>
  <div class="min-chart">
    <div class="min-current pnl-statis">
      ...
      <div v-if="intradayPnl !== ''">
        日内盈亏：
        <span
          :class="{
            'text-overflow': true,
            'color-green': intradayPnl < 0,
            'color-red': intradayPnl > 0,
          }"
          :title="intradayPnl"
        >
          {{ intradayPnl }}
    ...
    <tr-no-data
      v-if="intradayPnl === '' || minPnl.length == 0 || !rendererPnl"
    />
...
export default {
  ...
  watch: {
    ...
    minPnl(minPnlList, oldPnlMinList) {
      const { timeList, pnlDataList } = this.dealMinPnlList(
        minPnlList.slice(0),
      );
