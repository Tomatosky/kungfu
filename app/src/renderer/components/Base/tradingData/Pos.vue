...
export default {
  ...
  watch: {
    async kungfuData(positions) {
      const positionsResolved =
        this.dealPositionList(positions, this.searchKeyword) || {};
      ...
      const subscribedTickersStr = subscribePosTickers
        .slice(0)
        .sort((item1, item2) => {
      ...
      if (subscribePosTickers.length) {
        if (this.subscribedTickersStr !== subscribedTickersStr) {
          const isMdReady = await Promise.all(
            subscribePosTickers.map((item) => {
  ...
  methods: {
    ...
    dealPositionList(positions, searchKeyword) {
      ...
      if (!positionsAfterFilter.length)
        return {
          dataByKey: Object.freeze({}),
      ...
      positionsAfterFilter.kfForEach((item) => {
        ...
        positionData.yesterdayVolume = item.yesterdayVolume;
        positionData.todayVolume = item.todayVolume;
        positionData.totalVolume = item.totalVolume;
