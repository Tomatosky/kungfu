  ...
  <tr-dashboard :title="currentTitleResolved">
    <div slot="dashboard-header">
      ...
      <tr-dashboard-header-item v-if="!dateForHistory">
        <i
          class="el-icon-date mouse-over"
          title="历史"
          @click="dateRangeDialogVisiblityForHistory = true"
        ></i>
      </tr-dashboard-header-item>
      ...
      <tr-dashboard-header-item v-if="todayFinishPreSetting === undefined">
        <i
          class="el-icon-success mouse-over"
          v-if="todayFinish"
          title="查看未完成委托"
          @click="todayFinish = false"
        ></i>
        <i
          class="el-icon-circle-check mouse-over"
          v-else
          title="查看全部委托"
          @click="todayFinish = true"
        ></i>
      </tr-dashboard-header-item>

      <tr-dashboard-header-item>
        <i
          class="el-icon-download mouse-over"
          title="导出"
          @click="dateRangeDialogVisiblityForExport = true"
        ></i>
      </tr-dashboard-header-item>
      <tr-dashboard-header-item>
        <el-button
...
export default {
  ...
  data() {
    return {
      todayFinish:
        this.todayFinishPreSetting !== undefined
          ? this.todayFinishPreSetting
          : true,
  ...
  methods: {
    handleShowDetail(row) {
      ...
      orderData.accountId = orderData.accountId
        .split('手动')
        .join(' ')
        .split('任务')
        .join(' ')
    ...
    handleCancelAllOrders() {
      ...
      if (this.moduleType === 'account') {
        ...
        if (this.processStatus[this.gatewayNameResolved] !== 'online') {
          this.$message.warning(
            `需要先启动 ${this.gatewayNameResolved.toAccountId()} 交易进程！`,
          );
          return;
        }
      }
      ...
      this.$confirm(`确认全部撤单？`, '提示', {
        ...
        .then(() => {
          const orderDataList = watcher.ledger.Order.list().filter((item) => {
            return aliveOrderStatusList.includes(+item.status);
          });

          return kungfuCancelAllOrders(
            orderDataList,
            this.currentId,
            this.moduleType,
          );
    ...
    dealOrderList(orders, { todayFinish }) {
      ...
      if (!todayFinish) {
        ordersAfterFilter = ordersAfterFilter.filter((item) => {
          return aliveOrderStatusList.includes(+item.status);
        });
      }

      if (this.moduleType === 'strategy') {
        ordersAfterFilter = ordersAfterFilter.filter(
          (item) => Number(item.updateTimeNum) >= this.addTime,
        );
      }
      ...
      ordersAfterFilter.kfForEach((item) => {
        ...
        const orderStat = this.dateForHistory
          ? item
          : dealOrderStat(this.orderStat[orderId] || null);
      ...
      const ordersResolved = Object.values(orderDataByKey).sort((a, b) => {
