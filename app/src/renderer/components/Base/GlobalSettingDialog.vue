  ...
  >
    <div class="global-setting-content">
          ...
          >
            <ul>
              <li
                ...
                :key="set.key"
                :class="{
                  active: activeSettingItem === `${item.key}-${set.key}`,
                }"
      ...
      <div class="setting-content">
        <div
          class="setting-type-item"
          v-for="setting in settingConfig"
          :key="setting.key"
        >
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  v-if="
                    settingConfig[setting.key].value && config.type !== 'table'
                  "
                >
                  <el-input
                    ...
                    :type="item.key"
                    v-model="
                      settingConfig[setting.key].value[item.key][config.key]
                    "
                    ...
                    @change="
                      (e) =>
                        handleSwitchProcess(
                          e,
                          config,
                          settingConfig[setting.key].value[item.key],
                        )
                    "
                  ></el-switch>
                  ...
                  >
                    <i class="el-icon-document mouse-over"></i>
                    {{ item.name }}日志
                  </span>
                  ...
                  >
                    <i class="el-icon-folder mouse-over"></i>
                    打开whl文件夹
                  </span>
                  ...
                  <span
                    ...
                    @click.stop="
                      handleSelectFilePath(setting.key, item.key, config.key)
                    "
                  >
                    <i class="el-icon-folder mouse-over"></i>
                    选择目标路径
                ...
                <div class="setting-sub-item__table-content" v-else>
                  <ul class="table-content">
                    <li class="table-header">
                      <el-col
                        :span="cell.cellWidth || 3"
                        v-for="cell in config.row"
                        :key="cell.unique_key"
                      >
                      ...
                      >
                        ...
                        <el-col :span="1">
                          <i
                            ...
                            @click="
                              handleAddRow(config.target, config.row, index)
                            "
                          ></i>
                        </el-col>
                        <el-col
                          :span="1"
                          v-if="tables[config.target].length > 1"
                        >
                          <i
...
export default {
  ...
  mounted() {
    //设置高亮
    const firstKeyOfSystemConfig =
      Object.keys(this.settingConfig.system.config || {})[0] || '';
  ...
  methods: {
    ...
    handleSelectFilePath(settingKey, itemKey, configKey) {
      dialog.showOpenDialog(
        ...
        (filePath) => {
          ...
          this.settingConfig[settingKey].value[itemKey][configKey] =
            filePath[0];
    ...
    setActiveMenu() {
      ...
      const $settingItems = Array().slice.call(
        document.querySelectorAll('.global-setting-item'),
      );
    ...
    saveTables() {
      Object.keys(this.tablesSaveMethods || {}).forEach((key) => {
        ...
        //去掉无效的key
        const targetData = Object.values(targetDataResolve || {}).filter(
          (row) => {
            delete row.rowid;
            let i,
              len = filters.length;
        ...
        saveMethod(Object.freeze(targetData)).catch((err) =>
          console.error(err),
        );
      });
    },
