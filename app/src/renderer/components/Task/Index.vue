<template>
  <tr-dashboard :title="noTitle ? '' : '算法任务'">
    <div slot="dashboard-header">
      ...
      <tr-dashboard-header-item>
        <el-button size="mini" @click="handleAddTask" title="添加">
          添加
        </el-button>
      </tr-dashboard-header-item>
    </div>
      ...
      >
        ...
        >
          <template slot-scope="props">
            ...
            >
              {{
                (TaskTypeConfig[props.row.subType] || {}).name ||
                props.row.subType
              }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="状态" sortable show-overflow-tooltip>
          <template slot-scope="props">
            <tr-status
        ...
        <el-table-column
          ...
          min-width="180px"
        ></el-table-column>
        <el-table-column label="进程" sortable>
          <template slot-scope="props">
            <el-switch
        ...
        >
          <template slot-scope="props">
            <span
              class="tr-oper"
              @click.stop="handleOpenLogFile(props.row.processId)"
            >
              <i class="el-icon-document mouse-over" title="打开日志文件"></i>
            </span>
            <span
              class="tr-oper"
              @click.stop="handleOpenUpdateTaskDialog(props.row)"
            >
              <i class="el-icon-setting mouse-over" title="设置"></i>
            </span>
            <span
...
export default {
  ...
  methods: {
    ...
    handleConfirm(extSettingJSONString, configKey) {
      ...
      const processNameByUniKey = this.formUnikeyInProcessName(
        uniKey,
        extSettingData,
      );
      ...
      const cwd =
        process.env.NODE_ENV === 'production'
          ...
          : path.resolve(packageJSONPath, '..', 'lib');

      return this.preUpdate().then((res) => {
        ...
        return switchTask(true, {
          ...
          this.$store.dispatch(
            'setCurrentTask',
            this.buildTaskProcessItem(processName, processWithDetail),
          );
        });
      });
    },
