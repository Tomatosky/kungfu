...
export default {
  ...
  computed: {
    ...
    configListResolved() {
      if (!this.outsideAddTaskType) {
        ...
        const afterFilerConfigList = this.configList.filter(
          (item) => item.subType === this.outsideAddTaskType,
        );
    ...
    targetConfigIndex() {
      ...
      return configListResolved.findIndex(
        (item) => item.key === this.activeTabName,
      );
    },
  },
  ...
  methods: {
        ...
        if (valid) {
          ...
          this.$emit(
            ...
            JSON.stringify({
              ...
              subType:
                (this.configListResolved[this.targetConfigIndex] || {})
                  .subType || '',
            }),
            this.activeTabName,
          );
    ...
    getPostFormListByProps() {
      const configIndexByKey = this.getConfigIndexByKey(
        this.currentActiveConfigKey,
      );
      const configListResolved = this.configListResolved || [];
