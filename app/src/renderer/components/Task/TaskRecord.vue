  ...
  <tr-dashboard title="">
    <div slot="dashboard-header">
      <tr-dashboard-header-item width="320px">
        <el-select
          class="dashboard-header-selector"
          :value="currentTaskId"
          @change="handleSetCurrentTask"
        >
          <el-option
          ...
          >
            ...
            <tr-status :value="processStatus[task.processId]"></tr-status>
          </el-option>
        </el-select>
...
export default {
  ...
  mounted() {
    this.taskDataPipe = buildKungfuDataByAppPipe().subscribe(() => {
      const stateData = watcher.state;
      const timeValueList = ensureLedgerData(
        stateData.TimeValue.filter('tag_c', 'task').filter(
          'tag_a',
          this.currentTaskId,
        ),
        'update_time',
  ...
  computed: {
    currentConfigKey() {
      return (
        minimist(this.currentTask.args || '', this.taskExtMinimistConfig)
          .configKey || ''
      );
    },
    ...
    tableHeader() {
      return (
        this.targetConfig.displayConfig || this.getTaskDetailDefaultHeaders()
      );
    },
  ...
  methods: {
    ...
    handleSetCurrentTask(taskId) {
      this.$store.dispatch(
        'setCurrentTask',
        this.processStatusWithDetail[taskId],
      );
    },
