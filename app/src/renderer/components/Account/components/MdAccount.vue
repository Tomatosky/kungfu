<template>
  <tr-dashboard :title="title">
    ...
    <div class="table-body md-table">
      <el-table size="small" :data="mdList" height="100%" v-if="afterRender">
        <el-table-column prop="source_name" label="柜台" show-overflow-tooltip>
        ...
        <el-table-column label="状态" show-overflow-tooltip v-if="!isAdmin">
          <template slot-scope="props">
            <tr-status
              v-if="
                ifProcessRunning(
                  `md_${props.row.source_name}`,
                  processStatus,
                ) && processStatus[`md_${props.row.source_name}`] === 'online'
              "
              :value="(mdTdState[`md_${props.row.source_name}`] || {}).state"
            ></tr-status>
            <tr-status
              v-else-if="
                ifProcessRunning(
                  `md_${props.row.source_name}`,
                  processStatus,
                ) && processStatus[`md_${props.row.source_name}`] === 'stopping'
              "
              :value="processStatus[`md_${props.row.source_name}`]"
            ></tr-status>
            <tr-status v-else></tr-status>
          </template>
        </el-table-column>

        <el-table-column label="进程" v-if="!isAdmin">
          <template slot-scope="props">
            <span @click.stop>
              <el-switch
                :value="
                  ifProcessRunning('md_' + props.row.source_name, processStatus)
                "
                @change="handleMdSwitch($event, props.row)"
              ></el-switch>
            </span>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="props">
            <span
              class="tr-oper"
              @click.stop="handleOpenLogFile(`md_${props.row.source_name}`)"
            >
              <i class="el-icon-document mouse-over" title="打开日志"></i>
            </span>
            <span
              class="tr-oper"
              @click.stop="handleOpenUpdateAccountDialog(props.row)"
            >
              <i class="el-icon-setting mouse-over" title="行情源设置"></i>
            </span>
            <span
              :class="['tr-oper-delete', `delete-${props.row.source_name}`]"
              @click.stop="handleDeleteMd(props.row)"
            >
              <i class="el-icon-delete mouse-over" title="删除行情源"></i>
            </span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    handleDeleteMd(row) {
      ...
      this.$confirm(
        `删除行情源${source_name}会删除所有相关信息，确认删除吗？`,
        '提示',
        {
    ...
    handleMdSwitch(value, account) {
      return switchMd(account, value).then(({ type, message }) =>
