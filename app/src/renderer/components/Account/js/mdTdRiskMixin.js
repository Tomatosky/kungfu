...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['ifMasterLedgerRunning']),
  ...
  methods: {
    handleToggleKeepAllProcessRunning() {
      if (this.startAllProcessSwitchLoopController) {
        this.$message.warning(
          `请等待 ${this.targetTipTxt} ${this.tdmdType} 进程结束`,
        );
        return;
      }
      ...
      this.allProcessSwitchTimer = setTimerPromiseTask(() => {
        ...
        if (count >= 3) {
          this.$message.warning(
            `${this.targetTipTxt} ${this.tdmdType} 进程未完成，请手动 ${this.targetTipTxt}`,
          );
          this.clearAllProcessSwitchTimer();
        ...
        if (this.targetStatus === this.allProcessRunning) {
          this.$message.success(
            `${this.targetTipTxt} ${this.tdmdType} 进程 完成`,
          );
          this.clearAllProcessSwitchTimer();
          return Promise.resolve(false);
        }
        ...
        this.$message.info(
          `正在尝试 ${this.targetTipTxt} ${this.tdmdType} 进程 ${loopTxt}`,
        );
    ...
    clearAllProcessSwitchTimer() {
      this.allProcessSwitchTimer &&
        this.allProcessSwitchTimer.clearLoop &&
        this.allProcessSwitchTimer.clearLoop();
