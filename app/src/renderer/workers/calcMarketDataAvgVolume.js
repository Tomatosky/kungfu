...
self.addEventListener('message', (e) => {
  ...
  const targetFilenamesFromDays = buildDaysDate(days).map((name) =>
    path.resolve(dataPath, `${name}.csv`),
  );
  ...
  Promise.all(promises).then((quotesList) => {
...
function calcAvgVolume(quotesList) {
  ...
  quotesList.forEach((quotes) => {
    const quotesVolumeData = buildVolumeDataFromArrayById(
      quotes,
      'instrumentId, exchangeId',
    );

    Object.keys(quotesVolumeData || {}).forEach((key) => {
  ...
  Object.keys(avgVolumeData || {}).forEach((key) => {
