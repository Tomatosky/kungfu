  ...
  methods: {
    handleConfirmDateRangeForExportAllTradingData({ date, dateType }) {
      ...
      return getKungfuDataByDateRange(date, dateType)
        .then((kungfuData) => {
          ...
          const orderStat = ensureLedgerData(
            kungfuData.OrderStat,
            'insert_time',
          );
        ...
        pm2_bus.on('process:msg', (packet) => {
          ...
          switch (dataType) {
            case 'TIME_ALERT':
              ...
              if (!quoteAlive) {
                this.$message.warning(
                  `距离算法任务 ${processName} 开始执行还有 ${minute} 分钟，目前还未收到订阅行情，请检查交易进程与行情进程运行`,
                );
              } else {
                this.$message.info(
                  `距离算法任务 ${processName} 开始执行还有 ${minute} 分钟，请保证交易进程与行情进程运行`,
                );
              }
              break;
            ...
            case 'REQ_HIS_AVG_VOLUME': //历史均成交量
              ...
              sendDataToProcessIdByPm2('HIS_AVG_VOLUME', pm2Id, processName, {
                avgVolume: this.marketAvgVolume[days] || {},
              });
              break;
            ...
            case 'REQ_RECORD_DATA':
              ...
              const kungfuTimeNowStr = kungfu
                .formatTime(kungfuTimeNow, '%H:%M:%S.%N')
                .slice(0, 12);
    ...
    bindIPCListener() {
      ...
      ipcRenderer.on('ipc-emit-accountsAsset', (event, { childWinId }) => {
        const childWin = BrowserWindow.fromId(childWinId);
        childWin.webContents.send(
          'ipc-res-accountsAsset',
          Object.freeze(this.accountsAsset),
        );
      });

      ipcRenderer.removeAllListeners('ipc-emit-cancelOrder');
      ipcRenderer.on(
        'ipc-emit-cancelOrder',
      ...
      ipcRenderer.on('ipc-emit-kungfuConfig', (event, { childWinId }) => {
        const childWin = BrowserWindow.fromId(childWinId);
        return this.$store.dispatch('getKungfuConfig').then((kfConfig) => {
          childWin.webContents.send(
            'ipc-res-kungfuConfig',
            Object.freeze(kfConfig),
          );
        });
      });
