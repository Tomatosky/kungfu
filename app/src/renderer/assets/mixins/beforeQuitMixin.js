...
export default {
  ...
  computed: {
    ...
    showBeforeQuitDialog() {
      return (
        this.recordBeforeQuitLoading !== undefined &&
  ...
  methods: {
    ...
    recordBeforeQuit() {
      ...
      return Promise.all([this.recordQuote()]).finally(() => {
        this.recordBeforeQuitLoading = false;
      });
    },

    recordQuote() {
      const tickerIds = this.flatternTickers
        .map((item) => `${item.instrumentId}_${item.exchangeId}`)
        .join(',');
      const subscribedQuotes = watcher.ledger.Quote.list()
        .filter(
          (item) =>
            !!tickerIds.includes(`${item.instrument_id}_${item.exchange_id}`),
        )
      ...
      return writeCSV(
        path.join(KF_DATASET_QUOTE_DIR, `${fileName}.csv`),
        subscribedQuotes,
      );
    },
  },
