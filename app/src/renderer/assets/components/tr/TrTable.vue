          ...
          >
            ...
            >
              <template v-if="column.type === 'number'">
                <tr-blink-num
                  :theKey="`${column.prop}_${item[keyField]}_${
                    item[column.prop]
                  }`"
            ...
            <ul class="tr-table-row">
              ...
              >
                ...
                <template v-else-if="column.type !== 'operation' && renderHtml">
                  <span
                    class="tr-table-cell__html-render"
                    v-html="item[column.prop]"
                  ></span>
                </template>
                <template v-else-if="column.type === 'operation'">
...
export default {
  ...
  computed: {
    headerWidth() {
      ...
      const widthForFlex =
        this.bodyWidth -
        (widths.length ? sum(widths.map((item) => parseFloat(item.width))) : 0);
      const unit = (widthForFlex - 16) / flexWidthUnits;
      let headerWidthCollection = {};
      [...widths, ...flexs].forEach((item) => {
        headerWidthCollection[item.prop] = item.width
          ? item.width
          : unit * (item.flex || 1) + 'px';
      });
  ...
  mounted() {
    ...
    let $scrollerTable =
      this.$refs['tr-scroller-table'] ||
      this.$refs['tr-dynamic-scroller-table'];
  ...
  methods: {
    ...
    triggerToBottom() {
      return this.$nextTick().then(() => {
        ...
        $scrollerTable &&
          $scrollerTable.$el &&
          ($scrollerTable.$el.scrollTop = 1000000000);
      });
    },
