  ...
  <MainContent>
    <div class="account-content">
      <el-row style="height: 55%">
        <el-col :span="14">
          <el-tabs
            :value="currentAccountTabName"
            type="border-card"
            @tab-click="handleAccountTabClick"
          >
            <el-tab-pane
              :lazy="true"
              :label="getCurrentAccountTabLabelName('tdList')"
              name="tdList"
            >
              <TdAccount></TdAccount>
            </el-tab-pane>
            <el-tab-pane
              :lazy="true"
              :label="getCurrentAccountTabLabelName('mdList')"
              name="mdList"
            >
              <MdAccount></MdAccount>
            </el-tab-pane>
            <el-tab-pane
              :lazy="true"
              :label="getCurrentAccountTabLabelName('marketdata')"
              name="marketdata"
            >
              <MarketData
                :currentId="currentId"
            ...
            <el-tab-pane
              :lazy="true"
              :label="getCurrentAccountTabLabelName('holdInstruments')"
              name="holdInstruments"
            >
              <Pos
                :noTitle="true"
                moduleType="acocunt"
            ...
            <el-tab-pane
              :lazy="false"
              v-if="proMode"
              :label="getCurrentAccountTabLabelName('tradingTask')"
              name="tradingTask"
            >
        ...
        <el-col :span="10">
          ...
          <el-row style="height: 55%" class="has-padding-bottom">
            <el-tabs type="border-card" v-model="currentTradesPnlTabNum">
              <el-tab-pane
                :lazy="true"
                :label="`成交记录 ${showCurrentIdInTabName(
                  currentTradesPnlTabNum,
                  'trades',
                )}`"
                name="trades"
              >
      ...
      <el-row style="height: 45%">
        <el-col :span="14">
          <el-tabs v-model="currentOrdesTabName" type="border-card">
            <el-tab-pane
              :lazy="true"
              :label="`全部委托 ${showCurrentIdInTabName(
                currentOrdesTabName,
                'orders',
              )}`"
              name="orders"
            ...
            <el-tab-pane
              :lazy="true"
              :label="`未完成委托 ${showCurrentIdInTabName(
                currentOrdesTabName,
                'unfinishedOrders',
              )}`"
            ...
            <el-tab-pane
              :lazy="true"
              v-if="proMode"
              :label="`算法任务记录 ${currentTaskIdInTab}`"
              name="taskDetail"
            >
              <TaskRecord
                :currentId="currentId"
        ...
        <el-col :span="6">
          <MakeOrderDashboard></MakeOrderDashboard>
        </el-col>
      </el-row>
    </div>
  </MainContent>
...
export default {
    ...
    this.tradingDataPipe = buildKungfuDataByAppPipe().subscribe(() => {
      ...
      window.requestIdleCallback(
        () => {
          ...
          statTime('deal quote');
          const quoteList = watcher.ledger.Quote.list()
            .filter(
              (item) =>
                !!this.subscribedQuoteIds[
                  `${item.exchange_id}_${item.instrument_id}`
                ],
            )
  ...
  methods: {
    ...
    getCurrentAccountTabLabelName(name) {
      ...
      const isHoldInstrumentActive =
        this.currentAccountTabName === 'holdInstruments';

      switch (name) {
        case 'tdList':
          return isHoldInstrumentActive
            ? '账户列表'
            : `账户列表 ${this.currentId || ''}`;
        case 'mdList':
          return '行情源';
        ...
        case 'holdInstruments':
          return !isActive
            ? '持有标的'
            : `持有标的 ${(this.currentTickerResolved || {}).id || ''}`;
        case 'tradingTask':
          return !isActive
            ? '算法任务'
            : `算法任务 ${this.currentTaskId || ''}`;
      }
    },
      ...
      if (!this.currentTicker || !this.currentTicker.instrumentId) {
        if (tickerList.length) {
          const tickerListSort = tickerList.slice(0).sort((a, b) => {
            ...
            return result === 0
              ? ad.toString().localeCompare(bd.toString())
              : result;
          });
    ...
    dealTradingData() {
      ...
      const orderStat = getOrderStatByDest(
        ledgerData.OrderStat,
        'dest',
        this.currentLocationUID,
      );
      ...
      this.positions = Object.freeze(
        ledgerData.Position.filter('ledger_category', 0)
      ...
      if (this.currentTradesPnlTabNum == 'pnl') {
        this.pnl = ledgerData.AssetSnapshot.filter('ledger_category', 0)
          .filter('dest', this.currentLocationUID)
          .sort('update_time')
          .map((item) => Object.freeze(dealSnapshot(item)));

        this.dailyPnl = ledgerData.DailyAsset.filter('ledger_category', 0)
          .filter('dest', this.currentLocationUID)
          .sort('update_time')
          .map((item) => Object.freeze(dealSnapshot(item)));
      }
    ...
    dealTradingDataByTiker() {
      ...
      const allPositions = ledgerData.Position.nofilter('volume', 0)
        .list()
        .map((item) => Object.freeze(dealPos(item)));
      const positionsByTicker = transformTradingItemListToData(
        allPositions,
        'ticker',
      );
      this.positionsByTicker = Object.freeze(
        transformPositionByTickerByMerge(positionsByTicker, 'account') || [],
      );
      this.initSetCurrentTicker(this.positionsByTicker);

      if (this.moduleType === 'ticker' && this.currentTickerId) {
        const positionsByTickerForAccount =
          positionsByTicker[this.currentTickerId];
