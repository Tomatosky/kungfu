...
export default {
  ...
  methods: {
    ...
    handleShowOrCloseMakeOrderDashboard(orderInput) {
      return this.buildMakeOrderWin(orderInput).then(() => {
        this.emitCurrentMakeOrderWinInfo(orderInput);
        window.makeOrderWin &&
          window.makeOrderWin.show &&
          window.makeOrderWin.show();
        window.makeOrderWin &&
          window.makeOrderWin.focus &&
          window.makeOrderWin.focus();
      });
    },
    ...
    buildMakeOrderWin(orderInput) {
      //仅在strategy时创建窗口
      if (
        (!window.makeOrderWin || this.isMakeOrderWinIsDestroyed()) &&
        this.moduleType === 'strategy'
      ) {
        return openVueWin('makeOrder', `/make-order`, remote, {
          width: 470,
          height: 520,
        }).then((win) => {
    ...
    isHistoryData(type) {
      if (type === 'order') {
        return !!(
          this.historyData['order'] && (this.historyData['order'] || {}).date
        );
      } else if (type === 'trade') {
        return !!(
          this.historyData['trade'] && (this.historyData['trade'] || {}).date
        );
      } else {
    ...
    getHistoryData(type) {
      if (type === 'order') {
        return Object.freeze(
          (this.historyData['order'].data || []).map((item) => {
            return Object.freeze({
              ...item.orderStats,
              ...dealOrder(item),
            });
        ...
        return (this.trades = Object.freeze(
          (this.historyData['trade'].data || []).map((item) => {
            return Object.freeze({
              ...item.orderStats,
              ...dealTrade(item),
            });
