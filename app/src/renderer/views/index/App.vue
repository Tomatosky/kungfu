...
import {
  buildGatewayStatePipe,
  buildKungfuDataByAppPipe,
...
export default {
  ...
  beforeDestroy() {
    ...
    this.kungfuGatewayStateObserver &&
      this.kungfuGatewayStateObserver.unsubscribe();
    this.kungfuGloablDataObserver &&
  ...
  methods: {
    bindTradingDataListener() {
      buildKungfuDataByAppPipe().subscribe(() => {
        ...
        const accountAssets = ledgerData.Asset.filter('ledger_category', 0)
          .list()
          .map((item) => dealAsset(item));
        const accountAssetsResolved = transformAssetItemListToData(
          accountAssets,
          'account',
        );
        this.$store.dispatch(
          'setAccountsAsset',
          Object.freeze(accountAssetsResolved),
        );

        const strategyAssets = ledgerData.Asset.filter('ledger_category', 1)
          .list()
          .map((item) => dealAsset(item));
        const strategyAssetsResolved = transformAssetItemListToData(
          strategyAssets,
          'strategy',
        );
        this.$store.dispatch(
    ...
    bindKungfuGlobalDataListener() {
      this.kungfuGloablDataObserver =
        buildKungfuGlobalDataPipeByDaemon().subscribe((data) => {
      ...
      this.kungfuGatewayStateObserver = buildGatewayStatePipe().subscribe(
        (data) => {
    ...
    getWatcherStatus() {
      let timer = setInterval(() => {
        ...
        const archiveFinished =
          window.archiveStatus !== 'online' &&
          window.archiveStatus !== undefined;
        const daemonStatus =
          this.processStatus.kungfuDaemon === 'online' && this.daemonIsLive;
