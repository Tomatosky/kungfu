...
export const resLedgerData = (
  ...
  const quotes = ensureLedgerData(
    ledger.Quote.filter('instrument_id', ticker),
  ).map((item: QuoteOriginData) => dealQuote(item));
  const orders = getTargetOrdersByParentId(ledger.Order, parentId);
  const positions = ensureLedgerData(ledger.Position).map(
    (item: PosOriginData) => dealPos(item),
  );
  const positionsResolved =
    transformTradingItemListToData(positions, 'account')[accountId] || [];
...
export const resQuoteData = (
  ...
  const quotes = ensureLedgerData(ledger.Quote)
    .filter((quote: QuoteOriginData) =>
      tickers.includes(`${quote.instrument_id}_${quote.exchange_id}`),
    )
...
export const resInstrumentInfo = (
  pm2Id: number,
  tickers: string,
  processName: string,
) => {
  ...
  watcher.step();
  const ledger = watcher.ledger;
  const instruments = ensureLedgerData(ledger.Instrument).filter(
    (item: InstrumentOriginData) =>
      tickers.includes(`${item.instrument_id}_${item.exchange_id}`),
  );

  sendDataToProcessIdByPm2('INSTRUMENT_DATA', pm2Id, processName, {
    instruments,
  });
};

export const resPosData = (
  pm2Id: number,
  accountId: string,
  processName: string,
) => {
