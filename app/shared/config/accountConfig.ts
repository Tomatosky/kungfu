...
const dealValidator = (accountConfigItems: AccountSettingItem[]) => {
  return accountConfigItems.map(
    (item: AccountSettingItem): AccountSettingItem => {
      if (!item['validator']) return item;
      const validators: any = VALIDATOR;
      item['validator'] = (item['validator'] || []).map(
        (validatorName: string): any => validators[validatorName],
      );
      return item;
    },
  );
  ...
  try {
    ...
    configs.forEach((c: any): void => {
      if (c.type === 'source') {
        ...
        let tdItemConfig: AccountSettingItem[] = deepClone(
          (config.td_config || []).filter(
            (configItem: AccountSettingItem) => !configItem.risk,
          ) || [],
        );
        let mdItemConfig: AccountSettingItem[] = deepClone(
          config.md_config || [],
        );
        let riskItemConfig: AccountSettingItem[] = deepClone(
          (config.td_config || []).filter(
            (configItem: AccountSettingItem) => configItem.risk,
          ) || [],
        );
        ...
        //sim md_config 为 []
        if (mdItemConfig.length || config.name === 'sim')
          mdSources[source] = mdAccoutSetting;
