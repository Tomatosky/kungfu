...
export const ensureNum = (num: number | bigint | string): number => {
  num = +Number(num).toString();
  if (Number.isNaN(num)) return 0;
  if (!Number.isFinite(num)) return 0;
  if (num === 1.7976931348623157e308) return 0;
  return +num;
};
...
export const originOrderTradesFilterByDirection = (
  direction: number,
  offset: number,
  side: number,
