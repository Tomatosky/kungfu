...
import {
  ...
  KF_CONFIG_PATH,
  KF_TARADING_CONFIG_DEFAULT_PATH,
  KF_TARADING_CONFIG_PATH,
  KF_ADMIN_PASSWORD_CONFIG_DEFAULT_PATH,
  KF_ADMIN_PASSWORD_CONFIG_PATH,
...
export const initConfig = () => {
  ...
  if (!fse.existsSync(KF_TARADING_CONFIG_PATH)) {
    ...
    const kfTradingConfigJSON = fse.readJsonSync(
      KF_TARADING_CONFIG_DEFAULT_PATH,
    );
  ...
  if (!fse.existsSync(KF_ADMIN_PASSWORD_CONFIG_PATH)) {
    ...
    const adminPasswordConfigJSON = fse.readJsonSync(
      KF_ADMIN_PASSWORD_CONFIG_DEFAULT_PATH,
    );
