...
import {
  ...
  ensureNum,
  ensureLedgerData,
  addTwoItemByKeyForReduce,
  avgTwoItemByKeyForReduce,
} from '__gUtils/busiUtils';
import {
  OffsetName,
  OrderStatus,
...
export const watcher: any = (() => {
  ...
  if (process.env.APP_TYPE === 'cli') {
    ...
    return kungfu.watcher(
      KF_RUNTIME_DIR,
      kungfu.formatStringToHashHex(id),
      true,
      true,
    );
  }
  ...
  if (process.env.APP_TYPE === 'daemon') {
    return kungfu.watcher(
      KF_RUNTIME_DIR,
      kungfu.formatStringToHashHex('kungfu_daemon'),
      bypassQuote,
      true,
    );
  }
  ...
  if (process.env.RENDERER_TYPE === 'admin') {
    return kungfu.watcher(
      KF_RUNTIME_DIR,
      kungfu.formatStringToHashHex('renderer_admin'),
      true,
      true,
    );
  }
...
export const writeKungfuTimeValue = (
  id: string,
  label: string,
  type: string,
  val: string,
...
export const transformOrderTradeListToData = (
  list: OrderOriginData[] | TradeOriginData[],
  dealFunc: Function,
) => {
  ...
  return list
    .kfForEachAsync((item: OrderOriginData | TradeOriginData) => {
      const source = +item.source;
      const tmpAccountLocation = tmpSourceIdData[source];
      const accountLocation =
        tmpAccountLocation || decodeKungfuLocation(source);
      ...
      const strategyLocation: KungfuLocation =
        tmpDestLocation || decodeKungfuLocation(dest);
...
export const transformOrderInputListToData = (
  list: OrderInputOriginData[],
  dealFunc: Function,
) => {
  ...
  return list
    .kfForEachAsync((item: OrderOriginData | TradeOriginData) => {
      ...
      const strategyLocation: KungfuLocation =
        tmpStrategyLocation || decodeKungfuLocation(source);
      if (strategyLocation && strategyLocation.name) {
...
export const transformPositionByTickerByMerge = (
  positionsByTicker: { [propname: string]: PosData[] },
  type: string,
) => {
  const positionsByTickerList = Object.keys(positionsByTicker)
    .map((key: string) => {
      const tickerList: PosData[] = positionsByTicker[key];
      const tickerListResolved = tickerList.filter((item) => {
      ...
      return tickerListResolved.reduce((item1: PosData, item2: PosData) => {
        return {
          ...
          yesterdayVolume: addTwoItemByKeyForReduce(
            item1,
            item2,
            'yesterdayVolume',
          ).toString(),
          ...
          totalMarketPrice: addTwoItemByKeyForReduce(
            item1,
            item2,
            'totalMarketPrice',
          ),
          unRealizedPnl: addTwoItemByKeyForReduce(
            item1,
            item2,
            'unRealizedPnl',
          ),
...
export const transformAssetItemListToData = (list: any[], type: string) => {
  ...
  Object.keys(accountIdClientIdData || {}).forEach((id: string) => {
    const valueData = accountIdClientIdData[id].reduce((a: any, b: any) => {
      return {
        ...a,
        ...b,
      };
...
export const flatternOrderTrades = (list: any[]) => {
  ...
  list.kfForEach((item: any) => {
    ...
    const parent_id =
      item.parent_id === undefined ? item.parent_order_id : item.parent_id;
...
export const getOrderInputBySourceDest = (
  OrderInput: any,
  type: string,
  sourceDest: number,
) => {
  if (type === 'source') {
    return ensureLedgerData(
      OrderInput.filter('source', sourceDest),
      'insert_time',
    )
      .slice(0, 100)
    ...
    return ensureLedgerData(
      OrderInput.filter('dest', sourceDest),
      'insert_time',
    )
      .slice(0, 100)
      .map((item: OrderInputOriginData) => dealOrderInput(item));
...
export const getOrdersBySourceDestInstrumentId = (
  Order: any,
  type: string,
  sourceDestInstrumentId: number | string,
  exchnageId?: string,
  directionOrigin?: number,
) => {
  if (type === 'source') {
    return ensureLedgerData(
      Order.filter('source', sourceDestInstrumentId),
      'update_time',
    )
      .slice(0, 100)
    ...
    return ensureLedgerData(
      Order.filter('dest', sourceDestInstrumentId),
      'update_time',
    )
      .slice(0, 100)
      .map((item: OrderOriginData) => dealOrder(item));
    ...
    return ensureLedgerData(
      Order.filter('instrument_id', sourceDestInstrumentId).filter(
        'exchange_id',
        exchnageId,
      ),
      'update_time',
    ...
    console.error(
      'getOrdersBySourceDestInstrumentId type is not source, dest or instrument',
    );
    return [];
  }
};

export const getTradesBySourceDestInstrumentId = (
  Trade: any,
  type: string,
  sourceDestInstrumentId: number | string,
  exchnageId?: string,
  ...
  if (type === 'source') {
    return ensureLedgerData(
      Trade.filter('source', sourceDestInstrumentId),
      'trade_time',
    )
      .slice(0, 100)
    ...
    return ensureLedgerData(
      Trade.filter('dest', sourceDestInstrumentId),
      'trade_time',
    )
      .slice(0, 100)
      .map((item: TradeOriginData) => dealTrade(item));
    ...
    return ensureLedgerData(
      Trade.filter('instrument_id', sourceDestInstrumentId).filter(
        'exchange_id',
        exchnageId,
      ),
      'update_time',
    ...
    console.error(
      'getTradesBySourceDestInstrumentId type is not source, dest or instrument',
    );
    return [];
  }
};

export const getOrderStatByDest = (
  OrderStat: any,
  type?: string,
  dest?: number,
) => {
  if (type === 'dest') {
    return ensureLedgerData(
      OrderStat.filter('dest', dest),
      'insert_time',
    ).slice(0, 500);
  } else {
    return ensureLedgerData(OrderStat, 'insert_time').slice(0, 500);
  }
};
...
export function getTargetOrdersByParentId(Orders: any, parentId: string) {
  return ensureLedgerData(Orders.filter('parent_id', BigInt(parentId))).map(
    (item: OrderOriginData) => dealOrder(item),
  );
}
...
function resolveSourceDest(source: number, dest: number): SourceDest {
  const kungfuLocationSource: KungfuLocation =
    decodeKungfuLocation(+source) || {};
...
function resolveAccountId(
  ...
  const { sourceName, sourceGroup, destGroup } = resolveSourceDest(
    source,
    dest,
  );
...
export const dealOrderInput = (item: OrderInputOriginData): OrderInputData => {
  ...
  return {
    ...
    updateTimeMMDD: kungfu
      .formatTime(insert_time, '%m/%d %H:%M:%S.%N')
      .slice(0, 18),
...
export const dealOrder = (item: OrderOriginData): OrderData => {
  ...
  return {
    ...
    //用订单写入时间
    updateTime: kungfu
      .formatTime(update_time || insert_time, '%H:%M:%S.%N')
      .slice(0, 12),
    updateTimeMMDD: kungfu
      .formatTime(update_time || insert_time, '%m/%d %H:%M:%S.%N')
      .slice(0, 18),
...
export const dealPos = (item: PosOriginData): PosData => {
  ...
  return {
    ...
    updateTimeMMDD: kungfu
      .formatTime(update_time, '%m/%d %H:%M:%S.%N')
      .slice(0, 18),
...
export const dealOrderStat = (
  ...
  const latencyTrade =
    trade_time && ack_time
      ? +toDecimal(Number(trade_time - ack_time) / 1000, 0)
      : 0;
  const latencyNetwork =
    ack_time && insert_time
      ? +toDecimal(Number(ack_time - insert_time) / 1000, 0)
      : 0;
  const latencySystem =
    insert_time && md_time
      ? +toDecimal(Number(insert_time - md_time) / 1000, 0)
      : 0;
...
export const dealGatewayStates = (
  gatewayStates: StringToStringObject,
): Array<MdTdState | {}> => {
...
export const dealQuote = (quote: QuoteOriginData): QuoteData => {
  return {
    id:
      quote.exchange_id +
      quote.source_id +
      quote.instrument_id.toString() +
      String(quote.data_time),
