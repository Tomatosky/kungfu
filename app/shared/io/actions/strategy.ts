...
export const switchStrategy = (
  ...
  return getStrategyById(strategyId).then(
    (curStrategy: any): Promise<MessageData> => {
      const notExistMessage: MessageData = {
        type: 'error',
        message: `${strategyId} is not existed!`,
      };
      if (!curStrategy.length)
        return new Promise((resolve) => resolve(notExistMessage));
      const strategyPath = curStrategy[0].strategy_path;
      const noStrategyPath: MessageData = {
        type: 'error',
        message: `该策略未绑定任何文件！`,
      };
      if (!strategyPath)
        return new Promise((resolve) => resolve(noStrategyPath));
