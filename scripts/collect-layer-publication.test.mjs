...
function completeFixture(root) {
  write(root, 'darwin', 'kungfu-tech-spec-4.0.0-alpha.0.tgz');
  write(root, 'darwin', 'kungfu-tech-storage-4.0.0-alpha.0.tgz');
  for (const platform of ['darwin-arm64', 'linux-x64', 'win32-x64']) {
    write(root, platform, `kungfu-tech-storage-${platform}-4.0.0-alpha.0.tgz`);
    write(root, platform, `kungfu_storage-4.0.0a0-cp313-${platform}.whl`);
    write(root, platform, 'kungfu-sdk-4.0.0-alpha.0.crate', 'same-crate');
  }
  ...
  write(root, 'darwin', 'Kungfu Episodes-4.0.0-alpha.0-arm64.dmg');
  write(root, 'linux', 'Kungfu Episodes-4.0.0-alpha.0-x86_64.AppImage');
  write(root, 'win32', 'Kungfu Episodes Setup 4.0.0-alpha.0.exe');
}

test('collects the exact 15-file cross-platform publication set', () => {
...
test('rejects a platform package basename with divergent bytes', () => {
  ...
  try {
    const input = path.join(root, 'input');
    completeFixture(input);
    write(root, 'input/other', 'kungfu-sdk-4.0.0-alpha.0.crate', 'other');
    const result = spawnSync(
      process.execPath,
